...
def secretary_program_start():
    ...
    while True:
        ...
        if user_command == 'p':
            ...
            add_new_doc_shelf_number = add_new_doc(new_doc_number, new_doc_type, new_doc_owner_name, new_doc_shelf_number)
            print('\nНа полку "{}" добавлен новый документ:'.format(add_new_doc_shelf_number))
        elif user_command == 'd':
            user_doc_number = input('Введите номер документа - ')
